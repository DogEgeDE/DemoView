(function () {
  const rules = {
    ...
    'operator-create': {
      title: '运营活动创建 - 业务规则',
      rules: [
        ['字段一致性', '运营后台创建集体研学活动的基本信息、活动设置与活动内容字段，与学校端创建活动保持一致。'],
        ['模板可用性', '仅可选择已启用且已授权的集体研学活动告家长书模板；引用后生成当前活动独立快照。'],
        ['阅读策略', '可配置“是否强制阅读”。开启时须设置 1—300 秒整数倒计时，默认 15 秒；关闭时家长可直接进入签字。'],
        ['活动快照', '模板内容、阅读策略和阅读时长均随活动保存，后续模板变更不影响已创建活动。']
      ]
    },
    parent: {
      title: '活动报名与签署 - 业务规则',
